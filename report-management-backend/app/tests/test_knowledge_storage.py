...
def test_path_generation_schemas():
    ...
    assert doc_path == f"knowledge/organization/collection/original/{collection_id}/{document_id}_v2_document.pdf"

    # Test archive path
    arch_path = knowledge_storage_service.generate_archive_path(collection_id, document_id, filename, version=1)
    assert arch_path.startswith(f"knowledge/archive/{collection_id}/{document_id}_v1_")

    # Test export path
    export_id = uuid4()
    exp_path = knowledge_storage_service.generate_export_path(collection_id, "zip", export_id)
    assert exp_path == f"knowledge/processed/exports/{collection_id}_{export_id}.zip"

    # Test log path
    now = datetime(2026, 7, 16, 12, 0, 0, tzinfo=timezone.utc)
    log_path = knowledge_storage_service.generate_log_path("ingestion", now)
    assert log_path == "knowledge/logs/ingestion/2026-07-16_1784203200.log"
