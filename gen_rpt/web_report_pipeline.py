...
- 覆盖 top-down sizing、bottom-up sizing、adoption funnel、价值池/ROI、投资/融资、产能/项目进展、成本/经济性、监管/采用门槛中的至少六类。
...
- Cover at least six of: top-down sizing, bottom-up sizing, adoption funnel, value pool/ROI, investment/funding, capacity/project progress, cost/economics, regulation/adoption gates.
...
- 必须采用 hypothesis-driven 写作：每个关键判断要能回到一个待验证假设、支持证据或证据缺口。
- 必须纳入 market sizing 思路：用 top-down、bottom-up、adoption funnel、value pool 或 supply-side sizing 中至少 2 种方法解释市场空间，但不得编造缺失变量。
...
- exhibits 3-6 个；如果提出图表草稿，只能使用证据台账或事实包中的数字、年份、来源计数或同单位可比数据，必须保留 data_basis；不要使用方向性评分或内部综合指数。
...
- Use hypothesis-driven writing: every material claim should trace back to a testable hypothesis, supporting evidence or an explicit evidence gap.
- Include market-sizing logic: use at least two of top-down, bottom-up, adoption funnel, value pool or supply-side sizing to explain the opportunity, without inventing missing variables.
